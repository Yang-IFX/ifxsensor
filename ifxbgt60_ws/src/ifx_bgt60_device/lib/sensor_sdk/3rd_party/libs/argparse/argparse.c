...
/* disable warnings about unsafe functions with Visual studio */
#ifdef _MSC_VER
...
void
argparse_usage_stream(struct argparse* self, FILE* fh)
{
    if (self->usages) {
        fprintf(fh, "Usage: %s\n", *self->usages++);
    ...
    for (; options->type != ARGPARSE_OPT_END; options++) {
        ...
        if (options->type == ARGPARSE_OPT_GROUP) {
            fputc('\n', fh);
            fprintf(fh, "%s", options->help);
            fputc('\n', fh);
            continue;
        }
        pos = fprintf(fh, "    ");
        if (options->short_name) {
            pos += fprintf(fh, "-%c", options->short_name);
        }
        if (options->long_name && options->short_name) {
            pos += fprintf(fh, ", ");
        }
        if (options->long_name) {
            pos += fprintf(fh, "--%s", options->long_name);
        }
        if (options->type == ARGPARSE_OPT_INTEGER) {
            pos += fprintf(fh, "=<int>");
